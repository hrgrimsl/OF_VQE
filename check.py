...
molecule.filename = "test"
molecule = openfermionpsi4.run_psi4(molecule, run_scf=run_scf, run_mp2=run_mp2, run_cisd=run_cisd, run_ccsd=run_ccsd, run_fci=run_fci, delete_input=0)
...
[fci_e, fci_v] = scipy.sparse.linalg.eigs(hamiltonian,1, which='SR')
fci_gs = scipy.sparse.csc_matrix(fci_v[:,0]).transpose()
assert(np.isclose(fci_e[0].imag,0))
fci_e = fci_e.real
print(" FCI energy: %12.8f" %fci_e[0])

print(" Build particle number operators")
...
n_spat_orbs = len(alpha_occ)+len(alpha_vir)
n_spin_orbs = molecule.n_qubits
...
print(" Form Number operators")
for p in alpha_orbs:
    fop = openfermion.FermionOperator(((p,1),(p,0)))
    fermi_ops_na.append(fop)
for p in beta_orbs:
    fop = openfermion.FermionOperator(((p,1),(p,0)))
    fermi_ops_nb.append(fop)


for op in fermi_ops_na:
    qubit_ops_na.append(openfermion.transforms.get_sparse_operator(op, n_qubits = molecule.n_qubits))
for op in fermi_ops_nb:
    qubit_ops_nb.append(openfermion.transforms.get_sparse_operator(op, n_qubits = molecule.n_qubits))

na = 0
nb = 0
for p in range(n_spat_orbs):
    pna = fci_gs.conj().transpose().dot(qubit_ops_na[p].dot(fci_gs))[0,0].real
    pnb = fci_gs.conj().transpose().dot(qubit_ops_nb[p].dot(fci_gs))[0,0].real
    print(" Orbital %4i Na = %12.8f, Nb = %12.8f" %( p, pna , pnb))
    na += pna
    nb += pnb
...
for i in range(len(s)):
    ui = U[:,i]
    #print(ui)
    nai = ui.H.dot(ss_na.dot(ui))
    nbi = ui.H.dot(ss_nb.dot(ui))
    if s[i] < 1e-14:
        continue
    assert(np.isclose(nai.imag,0))
    assert(np.isclose(nbi.imag,0))
    nai = nai.real
    nbi = nbi.real
