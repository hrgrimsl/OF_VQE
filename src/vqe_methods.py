...
def q_adapt_vqe_bk(geometry,
    ...
    hamiltonian_jw = openfermion.transforms.get_sparse_operator(hamiltonian_op)
    # print("H_bk",hamiltonian)
    # print("H_jw", hamiltonian_jw)
    #  print(hamiltonian[:,240])
    HJW = openfermion.transforms.jordan_wigner(hamiltonian_op)
    # print(HJW)
    # pickle.dump(HJW, open('./h4_hamiltonian.p','wb'))
    ...
    HF_energy = reference_ket.transpose().conj().dot(hamiltonian.dot(reference_ket))
    print('HF energy', HF_energy)

    reference_ket_jw = scipy.sparse.csc_matrix(
        openfermion.jw_configuration_state(
            list(range(0, molecule.n_electrons)), molecule.n_qubits)).transpose()

    HF_energy_jw = reference_ket_jw.transpose().conj().dot(hamiltonian_jw.dot(reference_ket_jw))
    print('HF energy jw', HF_energy_jw)
    ...
    for n_iter in range(0, adapt_maxiter):
        ...
        trial_model = tUCCSD(hamiltonian, ansatz_mat, ansatz_ops, ansatz_mat_t, ansatz_ops_t, reference_ket, parameters,
                             parameters_index, parameters_mult, ansatz_sup_index, ansatz_left, ansatz_right,
                             ansatz_scal)
